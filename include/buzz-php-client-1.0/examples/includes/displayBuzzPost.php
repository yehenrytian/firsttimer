...
function displayBuzzPost($buzz, $post) {
  ...
  echo "
  		<div class=\"buzzPost ui-corner-all\">
    		<div class=\"thumbnail\"><a target=\"_blank\" href=\"{$post->person->profileUrl}\"><img width=45 height=45 src=\"{$post->person->thumbnailUrl}\"></a></div>
  			<div class=\"content\">
  				<a target=\"_blank\" class=\"person\" href=\"{$post->person->profileUrl}\">{$post->person->name}</a> $content
  				$articleContent";
  ...
  if (isset($post->comments) && count($post->comments)) {
    foreach ($post->comments as $comment) {
      ...
      echo "<div class=\"comment\"><a target=\"_blank\" class=\"person\" href=\"{$comment->person->profileUrl}\">{$comment->person->name}</a> {$comment->content}</div>";
    }
  }
