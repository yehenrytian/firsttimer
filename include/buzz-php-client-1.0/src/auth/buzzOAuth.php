...
class buzzOAuth {
   ...
   * @param buzzStorage $storage storage class to use (file,apc,memcache,mysql)
   ...
   */
  public function __construct(buzzStorage $storage, $localUserId) {
    global $buzzConfig;
    $buzzConfig['authorization_token_url'] = str_replace('www.example.org', OAuthUtil::urlencodeRFC3986($buzzConfig['site_name']), $buzzConfig['authorization_token_url']);
    $this->localUserId = $localUserId;
    $this->consumerToken = new OAuthConsumer($buzzConfig['oauth_consumer_key'], $buzzConfig['oauth_consumer_secret'], NULL);
    $this->signatureMethod = new OAuthSignatureMethod_HMAC_SHA1();
    $this->storage = $storage;
    $this->storageKey = 'OAuth:' . $buzzConfig['oauth_consumer_key'] . ':' . $localUserId; // Scope data to the local user as well, or else multiple local users will share the same OAuth credentials.
   ...
   * it uses the buzzStorage class to do so.
   *
   ...
   * @param buzzStorage $storage storage class to use (file,apc,memcache,mysql)
   * @param any $localUser the *local* user ID (this is not the user's ID on the social network site, but the user id on YOUR site, this is used to link the oauth access token to a local login)
   * @return buzzOAuth3Legged the logged-in provider instance
   */
  public static function performOAuthLogin(buzzStorage $storage, $localUserId = null, $redirect = true) {
    global $buzzConfig;
    $auth = new buzzOAuth($storage, $localUserId);
  ...
  public function upgradeRequestToken($requestToken, $requestTokenSecret, $oauthVerifier) {
    ...
    if ($ret['http_code'] == '200') {
      ...
      if (!isset($matches['oauth_token']) || !isset($matches['oauth_token_secret'])) {
        throw new buzzException("Error authorizing access key (result was: {$ret['data']})");
      }
      ...
      throw new buzzException("Error requesting oauth access token, code " . $ret['http_code'] . ", message: " . $ret['data']);
    }
  }
  ...
  protected function requestAccessToken($requestToken, $requestTokenSecret, $oauthVerifier) {
    ...
    $accessRequest = OAuthRequest::from_consumer_and_token($this->consumerToken, $accessToken, "GET", $buzzConfig['access_token_url'], array('oauth_verifier' => $oauthVerifier));
    $accessRequest->sign_request($this->signatureMethod, $this->consumerToken, $accessToken);
    return buzzIO::send($accessRequest, 'GET');
  }
  ...
  public function redirectToOriginal() {
    ...
    if ($originalUrl && !empty($originalUrl)) {
	  ...
	  //$this->FunJavaScriptRedirection($originalUrl);
    }
  }
  ...
  public function obtainRequestToken($callbackUrl, $uid) {
    ...
    if ($ret['http_code'] == '200') {
      ...
      if (!is_array($matches) || count($matches) != 4) {
        throw new buzzException("Error retrieving request key ({$ret['data']})");
      }
      return new OAuthToken(OAuthUtil::urldecodeRFC3986($matches[1]), OAuthUtil::urldecodeRFC3986($matches[2]));
    } else {
      throw new buzzException("Error requesting oauth request token, code " . $ret['http_code'] . ", message: " . $ret['data']);
    }
  }
  ...
  protected function requestRequestToken($callbackUrl) {
    global $buzzConfig;
    $requestTokenRequest = OAuthRequest::from_consumer_and_token($this->consumerToken, NULL, "GET", $buzzConfig['request_token_url'], array());
    $requestTokenRequest->set_parameter('scope', $buzzConfig['oauth_scope']);
    ...
    return buzzIO::send($requestTokenRequest, 'GET');
  }

  /**
   * Redirect the user to the (provider's) authorize page, if approved it should kick the user back to the call back URL
  ...
  public function redirectToAuthorization($token) {
    global $buzzConfig;
    $authorizeRedirect = $buzzConfig['authorization_token_url']. $token->key;
	//echo $authorizeRedirect;
    header("Location: $authorizeRedirect");
	//$this->FunJavaScriptRedirection($authorizeRedirect);
  }
  
  public function getAuthorizeURL() {
	global $buzzConfig;
	$token = $this->accessToken;
    $authorizeURL = $buzzConfig['authorization_token_url']. $token->key;
	return $authorizeURL;
  }
  ...
  **/
  public function FunJavaScriptRedirection($url) {
