$D = YAHOO.util.Dom;
$E = YAHOO.util.Event;
var yuiLoadingPanelUpload = function(conf){
    conf = conf == undefined ? new Array() : conf;
    conf.id = conf.id == undefined ? 'yuiLoadingPanel':confi.id;
    conf.header = conf.header == undefined ? 'Uploading your video, please wait...':conf.header;
    conf.width = conf.width == undefined ? '300px':conf.width;
    this.conf = conf;
...
yuiLoadingPanelUpload.prototype = {
    init:function(){
